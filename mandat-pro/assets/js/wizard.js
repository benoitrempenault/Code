...
(function () {
  ...
  function show(n) {
    ...
    try { sessionStorage.setItem("studio-mandatpro-step", String(current)); } catch (e) { }
  }
  ...
  function init() {
    ...
    wireAdText();
    let start = 1;
    try { start = parseInt(sessionStorage.getItem("studio-mandatpro-step"), 10) || 1; } catch (e) { }
    // Injection depuis la Fiche prestation (bouton « Injecter dans la brochure »)
    let injected = false;
    try {
      const raw = localStorage.getItem("studio-mandatpro-handoff");
      if (raw) {
        ...
        if (h && h.notes) {
          ...
          App().hydrateForm(); App().render(); App().save();
          injected = true;
        }
      }
    } catch (e) { }
    maxVisited = Math.max(1, start, injected ? 2 : 1);
    show(injected ? 2 : start);
    if (injected) App().toast("Fiche prestation injectée ✓ — vérifiez l'adresse puis lancez la recherche du quartier.");
    // Le paramétrage de l'agence se fait normalement à l'accueil — filet de sécurité ici.
