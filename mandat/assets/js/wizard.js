...
(function () {
  ...
  function show(n) {
    ...
    try { sessionStorage.setItem("studio-mandat-step", String(current)); } catch (e) { }
  }
  ...
  function init() {
    ...
    wireAdText();
    let start = 1;
    try { start = parseInt(sessionStorage.getItem("studio-mandat-step"), 10) || 1; } catch (e) { }
    // Injection depuis la Fiche prestation (bouton « Injecter dans la brochure »)
    let injected = false;
    try {
      const raw = localStorage.getItem("studio-mandat-handoff");
      if (raw) {
        localStorage.removeItem("studio-mandat-handoff");
