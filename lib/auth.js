...
function audit(userId, action, detail, ip, propertyId) {
  try {
    db.prepare('INSERT INTO audit_log (user_id, action, detail, ip, property_id) VALUES (?,?,?,?,?)').run(userId || null, action, detail || null, ip || null, propertyId || null);
  } catch (_) {}
}
